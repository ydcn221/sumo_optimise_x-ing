...
def _emit_vehicle_connections_for_approach(
    ...
    l = min(target_counts["L"], s_count)
    t = min(target_counts["T"], s_count)
    r = min(target_counts["R"], s_count)
    u = min(target_counts["U"], s_count)

    if s_count or any(target_counts.values()):
        LOG.info(
            "[LANE] approach targets pos=%s in_edge=%s s=%d targets=%s demand=%s",
            pos,
            in_edge_id,
            s_count,
            {
    ...
    LOG.info(
        "[LANE] allocated lanes pos=%s in_edge=%s labels=%s idx=%s",
        pos,
        in_edge_id,
        lane_labels,
        {"L": idx_l, "T": idx_t, "R": idx_r, "U": idx_u},
    )
    ...
    LOG.info(
        "[LANE] emitted %d vehicle connections pos=%s in_edge=%s",
        emitted,
        pos,
        in_edge_id,
    )
...
def render_connections_xml(
    ...
    for cluster in clusters:
        ...
        if exist_north:
            ...
            _emit_vehicle_connections_for_approach(
                lines, pos, in_edge, s_count, L_target, None, R_target, None
            )
