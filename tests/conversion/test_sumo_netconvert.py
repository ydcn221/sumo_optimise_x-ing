...
def test_run_two_step_netconvert_includes_tll_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Ensure the third netconvert call references the generated ``net.tll.xml`` file."""
    ...
    outdir = tmp_path
    nodes = outdir / constants.NODES_FILE_NAME
    edges = outdir / constants.EDGES_FILE_NAME
    connections = outdir / constants.CONNECTIONS_FILE_NAME
    tll = outdir / constants.TLL_FILE_NAME
    ...
    assert step3[step3.index("--tllogic-files") + 1] == constants.TLL_FILE_NAME
